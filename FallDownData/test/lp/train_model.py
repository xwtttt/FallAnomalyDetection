...
data = np.load('balance.npz')       #mark final.npz
label = data['y']
Bigdata = data['X']
...
for train, test in kfold.split(np.zeros(len(y)), y):
    ...
    keras.layers.Masking(mask_value=0., input_shape=(500, 7)), #mark 479
